...
このリポジトリは public です。`docs/blueprints/` に置いた設計図は、会員特典として
売っているものですが、GitHub 上では誰でも読めます。1本足すたびに読める本数が増えます。

扱いをどうするかは `docs/blueprints/README.md` の冒頭に選択肢を書きました。
オーナーが決めるまでは、この手順のまま続けてかまいません（すでに公開されている状態を
これ以上悪くしないため、下書きの本数を増やす以外のことはしないでください）。
