...
このリポジトリはまだ public です。`docs/blueprints/` に置いた設計図は会員特典ですが、
GitHub 上では誰でも読めます。**リポジトリを private にする方針が決まっています**
（オーナーが GitHub の Settings で切り替えます）。

切り替わるまでは、この手順のまま続けてかまいません。ただし設計図を1本足すごとに
無料で読める本数が増えることは頭に入れておいてください。
切り替わったら、この節と `docs/blueprints/README.md` `CLAUDE.md` の注意書きを消します。
